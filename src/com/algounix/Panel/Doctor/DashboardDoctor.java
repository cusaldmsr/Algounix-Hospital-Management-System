...
//                System.out.println(listModel);
    ...
    private void displayCharts() {
        chart.add(createPieChartPanel(createTodayAppointmentsDataset(), "Appointments Today"));
        chart.add(createPieChartPanel(createDoctorTypeDataset(), "Doctor Type Distribution"));

        chart.repaint();
        chart.revalidate();

        chart.setLayout(new java.awt.GridLayout(1, 1));
    }
    ...
    private PieDataset createTodayAppointmentsDataset() {
        ...
        try {
            ResultSet rs = MySQL.executeSearch("SELECT doctor_type.name AS type, COUNT(appoinment.id) AS total\n"
                    + "FROM appoinment\n"
                    + "INNER JOIN doctor_has_units ON appoinment.doctor_has_units_id = doctor_has_units.id\n"
                    + "INNER JOIN doctor ON doctor_has_units.doctor_id = doctor.id\n"
                    + "INNER JOIN doctor_type ON doctor.doctor_type_id = doctor_type.id\n"
                    + "GROUP BY doctor_type.name");
