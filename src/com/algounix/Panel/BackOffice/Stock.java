...
public class Stock extends javax.swing.JPanel {
  ...
  private String StockQuery = "SELECT * FROM `main_stock` "
    ...
    private void loadStock() {
        try {
            ...
            if (minPrice > 0 && maxPrice == 0) {
                query += " AND `main_stock`.`selling_price` >= '" + minPrice + "'";
            } else if (minPrice == 0 && maxPrice > 0) {
                query += " AND `main_stock`.`selling_price` <= '" + maxPrice + "'";
            } else if (minPrice > 0 && maxPrice > 0) {
                query += " AND `main_stock`.`selling_price` >= '" + minPrice + "' AND `main_stock`.`selling_price` <= '" + maxPrice + "' ";
            }
    ...
    private void loadLowStockAlerts() {
        try {
            ...
            while (resultSet.next()) {
                ...
                int qty = resultSet.getInt("received_qty");
    ...
    private void loadProducts() {
        ...
        try {
            ...
            ResultSet resultSet = MySQL.executeSearch("SELECT * FROM `medicine` "
                    + "INNER JOIN `brand` ON `medicine`.`brand_id` = `brand`.`id`"
                    + "INNER JOIN `category`ON `medicine`.`category_id` = `category`.`id` ORDER BY `medicine`.`id` ASC");
    ...
    private void initComponents() {
        ...
        jComboBox1.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Stock ID ASC", "Stock ID DESC", "Product Name ASC", "Product Name DESC", "Brand Name ASC", "Brand Name DESC", "Expire Date ASC", "Expire Date DESC", " " }));
