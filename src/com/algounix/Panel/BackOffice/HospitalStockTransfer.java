...
public class HospitalStockTransfer extends javax.swing.JPanel {

    public HospitalStockTransfer() {
        initComponents();
        loadHospitalStock();
        loadReceivedStocks();
        loadReqestedStocks();
    ...
    private void loadHospitalStock() {
        try {

            String query = "SELECT * FROM `main_stock` INNER JOIN `medicine` ON `main_stock`.`medicine_id` = `medicine`.`id`"
                    + "INNER JOIN `brand` ON `medicine`.`brand_id` = `brand`.`id`"
                    + "INNER JOIN `weight_unit` ON `medicine`.`weight_unit_id` = `weight_unit`.`id` WHERE (`transfered_status_id` = '1' OR `transfered_status_id` = '2')";
            ...
            if (sort.equals("Stock ID ASC")) {
                ...
                query += "`main_stock`.`selling_price` ASC";
            } else if (sort.equals("Price DESC")) {
                query += "`main_stock`.`selling_price` DESC";
                ...
                query += "`main_stock`.`qty` ASC";
            } else if (sort.equals("Quantity DESC")) {
                query += "`main_stock`.`qty` DESC";
            }
            ...
            while (rs.next()) {
                ...
                vector.add(rs.getString("main_stock.id"));
                ...
                vector.add(rs.getString("main_stock.selling_price"));
                vector.add(rs.getString("main_stock.mfd"));
                vector.add(rs.getString("main_stock.exp"));
                vector.add(rs.getString("main_stock.qty"));
    ...
    private void loadReceivedStocks() {
        try {
            ...
            ResultSet rs = MySQL.executeSearch("SELECT * FROM `main_stock` "
                    + "INNER JOIN `medicine` ON `main_stock`.`medicine_id` = `medicine`.`id` "
                    + "INNER JOIN `weight_unit` ON `medicine`.`weight_unit_id` = `weight_unit`.`id` WHERE `transfered_status_id` = '2'");
            ...
            while (rs.next()) {
                ...
                vector.add(rs.getString("main_stock.selling_price"));
                vector.add(rs.getString("main_stock.exp"));
                vector.add(rs.getString("main_stock.received_qty"));
    ...
    private void loadReqestedStocks() {
        ...
        try {
            ...
            ResultSet rs = MySQL.executeSearch("SELECT * FROM `stock_request` INNER JOIN `medicine` ON `stock_request`.`medicine_id` = `medicine`.`id`"
                    + " WHERE `request_from` = 'P' AND (`request_status_id`='2' OR `request_status_id`='1')");
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Hospital Stock Transfer");
        ...
        jLabel2.setText("SEND STOCKS TO PHARMACY");
        ...
        jLabel3.setText("REQUESTED STOCK FROM PHARMACY");

        jButton2.setFont(new java.awt.Font("Poppins", 1, 12)); // NOI18N
        jButton2.setText("Accept & Send");
        ...
        jButton3.setText("Decline Request");
        ...
        jLabel4.setText("RECEIVED STOCKS FROM PHARMACY");
        ...
        if (row != -1) {
            ...
            if (medID.isEmpty()) {
                ...
                try {
                    ...
                    double pharmacyPrice = Double.parseDouble(price) * 105 / 100;

                    //  Update Main Stock Table (Reduce Sended Qty)
                    MySQL.executeIUD("UPDATE `main_stock` SET `qty` = '" + String.valueOf(qty) + "' WHERE `main_stock`.`id` = '" + stockID + "'");

                    //  First We Need To check The received Stock is Adready Added To The Pharmacy Stock.
                    ResultSet rs = MySQL.executeSearch("SELECT * FROM `pharmacy_stock` WHERE `main_stock_id` = '" + stockID + "'");

                    if (rs.next()) {
                        //  If that Stock is Already Added We need To Update QTY of That Row
                        MySQL.executeIUD("UPDATE `pharmacy_stock` SET `transfered_date` = '" + today + "', "
                                + "`received_qty` = '" + sendingQty + "', `transfered_status_id` = '2' "
                                + "WHERE `main_stock_id` = '" + stockID + "'");
                    } else {
                        //  Add New Stock To Pharmacy Table (Add New Row to table With Received Status) Number 2 is Received Status
                        MySQL.executeIUD("INSERT INTO `pharmacy_stock` (`transfered_date`,`main_stock_id`,`received_qty`,`selling_price`,`transfered_status_id`,`have_qty`)"
                                + "VALUES ('" + today + "','" + stockID + "','" + sendingQty + "','" + pharmacyPrice + "','2','0')");
                    }
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if (row != -1) {
            ...
            try {
                ResultSet rs = MySQL.executeSearch("SELECT * FROM `main_stock` WHERE `id` = '" + stockId + "'");
                if (rs.next()) {

                    double qty = rs.getDouble("qty") + Double.parseDouble(receivedQty);
                    String status = "";
                    ...
                    LocalDate date2 = LocalDate.parse(rs.getString("exp"), formatter);
                    ...
                    if (date1.isBefore(date2)) {
                        ...
                        status = "3";
                        JOptionPane.showMessageDialog(this, "Stock is Expired. Remove from the Warehose", "Success", JOptionPane.INFORMATION_MESSAGE);
                    }

                    MySQL.executeIUD("UPDATE `main_stock` SET `qty` = '" + qty + "', `transfered_status_id` = '" + status + "', "
                            + "`received_qty` = '0', `last_updated_date` = '" + today + "' WHERE `id` = '" + stockId + "'");
            ...
            if (medID.isEmpty()) {
                ...
                try {
                    MySQL.executeIUD("INSERT INTO `stock_request` (`requested_date`,`requested_qty`,`medicine_id`,`request_from`,`request_status_id`)"
                            + "VALUES('" + today + "','" + reqQty + "','" + medID + "','H','" + selectedTpye + "')");
        ...
        if (id != null) {
            try {
                ...
                if (rs.next()) {
                    ...
                    JOptionPane.showMessageDialog(this, "Request Add to the Tranfer", "Warning", JOptionPane.INFORMATION_MESSAGE);
                } else {
        ...
        if (id != null) {
            try {
                ...
                if (rs.next()) {
                    ...
                    JOptionPane.showMessageDialog(this, "Request Declined.", "Warning", JOptionPane.INFORMATION_MESSAGE);
                } else {
