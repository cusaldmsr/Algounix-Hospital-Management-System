 ...
 * @author kusal
 */
public class DashboardLaboretory extends javax.swing.JPanel {
    ...
    private void displayCharts() {
        piechart.add(createPieChartPanel(createLabTestTypeDataset(), "Lab Tests by Test Type"));

        piechart.revalidate();
        piechart.repaint();

        piechart.setLayout(new java.awt.GridLayout(1, 1));
    ...
    //piechart 1 Dataset
    private PieDataset createLabTestTypeDataset() {
        ...
        try {
            ResultSet rs = MySQL.executeSearch("SELECT \n"
                    ...
                    + "    test_items ti\n"
                    + "INNER JOIN \n"
                    + "    test_list tl ON ti.test_list_id = tl.id\n"
                    + "INNER JOIN \n"
                    + "    test_type tt ON tl.test_type_id = tt.id\n"
                    + "GROUP BY \n"
                    + "    tt.name");

            while (rs.next()) {
                dataset.setValue(rs.getString("test_type"), rs.getInt("total"));
            }
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Laboretory Dashboard");
